...
int main(int argc, char** argv) {
	...
	// Select which kernel
	if (op == "add") {
		add_kernel_shr_i32<<<num_blocks, total_threads, 3*total_threads*sizeof(int)>>>(res, in1, in2, array_size);
	}
	else if (op == "sub") {
		sub_kernel_shr_i32<<<num_blocks, total_threads, 3*total_threads*sizeof(int)>>>(res, in1, in2, array_size);
	}
	else if (op == "mul") {
		mul_kernel_shr_i32<<<num_blocks, total_threads, 3*total_threads*sizeof(int)>>>(res, in1, in2, array_size);
	}
	else if (op == "mod") {
		mod_kernel_shr_i32<<<num_blocks, total_threads, 3*total_threads*sizeof(int)>>>(res, in1, in2, array_size);
	}
